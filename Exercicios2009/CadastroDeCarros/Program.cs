...
namespace CadastroDeCarros
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int w = 0; w < 10; w++)
            {
                //Gera um número aleatório de 0a 100
                int valorGradoAleatoriamente = rdm.Next(100);

                //verifica se a lista não contém este numero gerado
                if (!numeros.Contains(valorGradoAleatoriamente))
                {
                    //Adiciona o número gerado pelo random (que não é repetido)
                    numeros.Add(valorGradoAleatoriamente);
                }
                else
                {
                    w--;
